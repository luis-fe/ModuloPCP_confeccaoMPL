...
@Simulacao_prod_routes.route('/pcp/api/exluindo_simulacao_Produtos_zerados', methods=['DELETE'])
@token_required
def post_exluindo_simulacao_Produtos_zerados():

    data = request.get_json()
    nomeSimulacao = data.get('nomeSimulacao')
    arrayProdutoZerados = data.get('arrayProdutoZerados',[])
    arrayPercentualZerados = data.get('arrayPercentualZerados',[])
    empresa = data.get('empresa','1')



    dados = SimulacaoProg.SimulacaoProg(nomeSimulacao,'','','','',empresa).exluindo_simulacao_Produtos_zerados(arrayProdutoZerados, arrayPercentualZerados)
