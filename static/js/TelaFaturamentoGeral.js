...
async function Faturamento() {
    try {
        ...
        const response = await fetch(
            `http://192.168.0.183:8000/pcp/api/dashboarTV?ano=${anoSelecionado}`,
            {
                method: 'GET',
                headers: {
...
function criarTabelaEmbalagens(listaChamados, CondicaoFat, CondicaoMeta) {
    ...
    th.innerHTML = text;
    th.style.backgroundColor = 'rgb(25, 135, 84)'; // Cor success
...
// Preenchimento das linhas com dados
    listaChamados.forEach(item => {
        ...
        celula1.style.fontSize = '1rem'; // aumenta a fonte
        ...
        celula2.style.fontSize = '1rem'; // aumenta a fonte
        celula2.innerHTML = item[CondicaoMeta];

        const celula3 = linha.insertCell(2);
        celula3.style.fontSize = '1rem'; // aumenta a fonte
